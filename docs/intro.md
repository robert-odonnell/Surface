...
- `SurrealSession`: snapshot-isolated entity store. Sync reads, sync writes (mutate the in-memory snapshot), async dispatch (`SaveAsync` / `DeleteAsync` / `UnrelateAsync` / `Query…Async` traversal terminals) against an open `Transaction`.
...
5. For writes: open a transaction with `await using var tx = await db.BeginTransactionAsync();`, call `await session.SaveAsync(entity, tx)` per aggregate root (the per-entity Save dispatch walks forward refs, then the entity, then new children) and `await session.SaveAsync(new TVariant { Source = …, Target = … }, tx)` per relation edge, then call `await tx.CommitAsync()`. Concurrent writers collide at COMMIT as `Disruptor.Surreal.SurrealConflictException` — catch, reload, retry.
