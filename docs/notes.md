...
- **`IEntity`** — every `[Table]` class implements this implicitly via the emitted partial. Session-side hooks (all explicit-interface impls, so they don't pollute the user's type):
  ...
  - `Hydrate(SurrealValue row, IHydrationSink sink)` / `HydratePartial(SurrealValue row, IHydrationSink sink)` — loader-driven row-to-entity population, consumes the SDK's `Disruptor.Surreal.Values.SurrealValue` directly.
