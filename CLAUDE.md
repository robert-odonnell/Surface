...
4. **Emit** (`Emit/`) — eight emitters fire per generation:
   ...
   - `ReferenceRegistryEmitter` — sealed `IReferenceRegistry` impl in the consumer with a `[ModuleInitializer]` that registers it with the runtime's static `ReferenceRegistry` facade on assembly load.
   - `SchemaEmitter` — generates `GeneratedSchema.Script` (`IReadOnlyList<string>` of DDL chunks: writer_lease + entity tables block + one chunk per `[Table]`'s fields + one chunk per relation kind) so consumers don't hand-maintain a separate schema file. Idempotent via `DEFINE … IF NOT EXISTS`. Chunked layout lets callers run pieces in separate transactions, filter for migration scenarios, or log per-chunk progress.
...
- `ReferenceRegistryEmitter` can't span an assembly boundary as a partial, so it emits a sealed `IReferenceRegistry` impl (`GeneratedReferenceRegistry`) into the consumer with a `[ModuleInitializer]` that calls `Surface.Runtime.ReferenceRegistry.Register(Instance)` on assembly load. Anything new the runtime needs at commit time can follow the same pattern: define a static facade + interface in the runtime, generate a registered impl in the consumer.
...
- **`WriterLease`** — DB-level cross-process writer coordination. **Lives outside the session** — callers acquire and dispose it independently, and pass it into `SurrealSession.CommitAsync` for renewal. One record per aggregate (`writer_lease:design`, `writer_lease:review`, …) with `holder_id` + `acquired_at` + `expires_at`. 5-min default TTL. `AcquireAsync` runs a transactional check-then-set; `RenewAsync` (called from `CommitAsync` if a lease is supplied) bumps `expires_at` and detects theft; `ReleaseAsync` deletes the row on graceful dispose; expired leases get stolen by the next acquirer. Failure modes surface as `WriterLeaseUnavailableException` (acquire) and `WriterLeaseStolenException` (renew).
...
- Default id value type is `Ulid`. To override globally, add `[assembly: RecordIdValue<TCustom>]` — the generator currently has hard-coded factories for `Ulid` / `Guid` / `string`, so anything else falls through to a `NotImplementedException` until you extend `IdEmitter`.
...
The Sample project's classes (`Design`, `Constraint`, `Epic`, `Feature`, `UserStory`, `AcceptanceCriteria`, `Test`, `Review`, `Finding`, `Observation`, `Issue`, `DesignChange`, `Details`) are the canonical worked examples — read them alongside the generated `.g.cs` outputs (especially `Surface.Runtime.GeneratedSchema.g.cs`) to see the input/output mapping. The DDL is no longer hand-maintained: `SchemaEmitter` walks the model graph and emits `GeneratedSchema.Script`, an `IReadOnlyList<string>` whose first chunk is `WriterLease.SchemaScript` and whose remaining chunks are entity tables, per-`[Table]` fields, and per-relation tables — apply with `foreach (var chunk in GeneratedSchema.Script) await transport.ExecuteAsync(chunk);`.
