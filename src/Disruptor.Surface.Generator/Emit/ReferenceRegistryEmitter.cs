...
internal static class ReferenceRegistryEmitter
{
    ...
    public static void Emit(SourceProductionContext spc, ModelGraph graph)
    {
        ...
        sb.Append(memberIndent).AppendLine("/// <summary>The model's reference-field registry. Pass into <see cref=\"global::Disruptor.Surface.Runtime.SurrealSession\"/> to drive [Reject] / [Unset] / [Cascade] resolution at commit time.</summary>");
        sb.Append(memberIndent).Append("public static global::Disruptor.Surface.Runtime.IReferenceRegistry ReferenceRegistry => ").Append(GeneratedClass).AppendLine(".Instance;");
        sb.Append(indent).AppendLine("}");
        sb.AppendLine();

        // Impl class — internal to the consumer assembly.
        sb.Append(indent).Append("internal sealed class ").Append(GeneratedClass).AppendLine(" : global::Disruptor.Surface.Runtime.IReferenceRegistry");
        sb.Append(indent).AppendLine("{");
        ...
        sb.AppendLine();
        sb.Append(memberIndent).AppendLine("private static readonly System.Collections.Generic.Dictionary<string, System.Collections.Generic.IReadOnlyList<global::Disruptor.Surface.Runtime.ReferenceFieldInfo>> _byReferencedTable = new(System.StringComparer.Ordinal)");
        sb.Append(memberIndent).AppendLine("{");
        foreach (var kv in byReferenced)
        {
            sb.Append(memberIndent).Append("    [\"").Append(kv.Key).AppendLine("\"] = new global::Disruptor.Surface.Runtime.ReferenceFieldInfo[]");
            sb.Append(memberIndent).AppendLine("    {");
            foreach (var entry in kv.Value)
            {
                sb.Append(memberIndent).Append("        new(\"").Append(entry.Referencer).Append("\", \"").Append(entry.Field).Append("\", \"").Append(kv.Key).Append("\", ")
                  .Append("global::Disruptor.Surface.Runtime.ReferenceDeleteBehavior.").Append(entry.Behavior.ToString()).Append(", ")
        ...
        sb.Append(memberIndent).AppendLine("};");
        sb.AppendLine();
        sb.Append(memberIndent).AppendLine("public System.Collections.Generic.IReadOnlyList<global::Disruptor.Surface.Runtime.ReferenceFieldInfo> IncomingReferencesTo(string referencedTable) =>");
        sb.Append(memberIndent).AppendLine("    _byReferencedTable.TryGetValue(referencedTable, out var refs) ? refs : System.Array.Empty<global::Disruptor.Surface.Runtime.ReferenceFieldInfo>();");
        sb.Append(indent).AppendLine("}");
