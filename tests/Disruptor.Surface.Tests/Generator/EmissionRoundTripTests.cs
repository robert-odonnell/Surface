...
[Fact]
    public async Task LoadAsync_FilteredSlice_IncludedReadsSucceed_OutsideReadsThrow()
    {
        ...
        var task = (Task)loadAsync.Invoke(null, [query, transport, CancellationToken.None])!;
        await task;
    ...
    public async Task FetchAsync_OnClosedSession_Throws()
    {
        ...
        // Close the session via Abandon — sessions stay open across multiple per-entity
        // SaveAsync calls; Abandon is the explicit close.
        session.Abandon();
    ...
    public async Task LoadAsync_WithIncludes_UsesCompilerDrivenPath_AndHydratesSlice()
    {
        ...
        var task = (Task)loadAsync.Invoke(null, [withIncludes, transport, CancellationToken.None])!;
        await task;
        var session = (SurrealSession)task.GetType().GetProperty("Result")!.GetValue(task)!;
